...
let successstudent_fun = function (data) {
    ...
    if (result === 'student') {
        document.querySelector('main').innerHTML = MESSAGES['succeeded-student'] + '<br> <p><a href="#" onclick="window.location.reload(true)">' +
            MESSAGES['back'] + '</a></p>';
    } else {
        document.querySelector('main').innerHTML = MESSAGES['failed-student'] + '<br> <p><a href="#" onclick="window.location.reload(true)">' +
            MESSAGES['back'] + '</a></p>';
    }
};
...
let successschool_fun = function (data) {
    ...
    document.querySelector('main').innerHTML = '<br><p><a href=\"#\" onclick=\"window.location.reload(true)\">' + MESSAGES['back'] + '</a></p>';
    document.querySelector('main').prepend(MESSAGES['succeeded-school'](role, school));
};
